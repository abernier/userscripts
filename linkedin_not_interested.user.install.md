...
1. Ask them to confirm: "I have the Tampermonkey extension installed" (yes/no). If your environment gives you browser access (e.g. Claude in Chrome), verify it yourself instead of asking. If they don't have it, send them to the official download page https://www.tampermonkey.net/ — its Download section offers the right store link for their browser (Chrome, Firefox, Edge, Safari…); prefer stable Tampermonkey over Legacy/BETA variants. Do not hardcode a store URL yourself. On Chrome, make sure "Allow User Scripts" (or Developer mode) is enabled for Tampermonkey in `chrome://extensions`, otherwise scripts will not run.
